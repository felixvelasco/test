...
abstract class AbstractJaxwsMojo extends AbstractMojo {
	...
	/**
	 * Path to the executable. Should be either wsgen or wsimport but basically any script which will understand passed
	...
	/*
	 * Information about this plugin, used to lookup this plugin's configuration from the currently executing project.
	 * 
	 * @since 2.2.1
	 */
	// requires M3 ?
	...
	protected List<String> getCommonArgs() throws MojoExecutionException {
		...
		if (isArgSupported("-encoding")) {
			if (encoding != null) {
				...
				getLog().warn(
						"Using platform encoding (" + System.getProperty("file.encoding")
		...
		try {
			if (a != null) {
				...
				if (av.getMajorVersion() == 2 && av.getMinorVersion() == 2 && av.getIncrementalVersion() == 6) {
					...
					supportedArgs = METRO_221;
				} else { // if (av.getMajorVersion() >= 2 && av.getMinorVersion() >= 2 && av.getIncrementalVersion() >=
							// 8) {
					supportedArgs = METRO_23;
				}
			}
	...
	protected void exec(List<String> args) throws MojoExecutionException {
		...
		try {
			...
			if (executable != null) {
				...
				cmd.setExecutable(new File(new File(System.getProperty("java.home"), "bin"), getJavaExec())
						.getAbsolutePath());
			...
			if (isWindows() && 8191 <= fullCommand.length()) {
				getLog().warn(
						"Length of the command is limitted to 8191 characters but it has " + fullCommand.length()
								+ " characters.");
	...
	protected void maybeUnsupportedOption(String option, String value, List<String> args) {
		if (executable == null) {
			...
			getLog().warn(
					option + " may not supported on older JDKs.\n"
	...
	private String[] getCP() throws ArtifactResolutionException, ArtifactNotFoundException {
		...
		for (Plugin p : plugins) {
			if ("jaxws-maven-plugin".equals(p.getArtifactId()) && "com.vectorsf.jvoice".equals(p.getGroupId())) {
				...
				for (Dependency d : p.getDependencies()) {
					...
					if ("jaxws-tools".equals(d.getArtifactId()) && "com.sun.xml.ws".equals(d.getGroupId())
							|| "webservices-tools".equals(d.getArtifactId())
							&& "org.glassfish.metro".equals(d.getGroupId())) {
						toolsAdded = true;
					}
					...
					ArtifactResolutionResult res = artifactResolver.resolveTransitively(toInclude, originatingArtifact,
		...
		for (Artifact a : cp) {
			if ("jaxws-api".equals(a.getArtifactId()) || "jaxb-api".equals(a.getArtifactId())
					...
					|| "javax.annotation".equals(a.getArtifactId()) || "javax.annotation-api".equals(a.getArtifactId())
		...
		// add custom invoker
		String invokerPath = AbstractJaxwsMojo.class.getProtectionDomain().getCodeSource().getLocation()
				.toExternalForm();
		try {
