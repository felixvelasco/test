...
public abstract class SimpleStatePattern extends IdPattern
{
	...
	private void updateFireableEvents(IdLayoutContext context)
	{
		...
		if (fireableEvents.size() * IMAGE_SIZE > MAIN_RECTANGLE_WIDTH)
			Graphiti.getGaService().setLocationAndSize(ga, ga.getX(), ga.getY(), (IMAGE_SIZE+2) * fireableEvents.size(), ga.getHeight());
		else
		...
		for (int i = 0; i< fireableEvents.size(); i++)
		{
			...
			anchor.setLocation(getImageLocation(i, context.getRootPictogramElement()));
