...
public class ValidateListener implements ICellEditorListener {
	...
	public void editorValueChanged(boolean oldValidState, boolean newValidState) {
		if (!newValidState) {
			...
			error.setBackground(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell().getDisplay().getSystemColor(SWT.COLOR_RED));
		}else{
			error.setText("");
			error.setBackground(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell().getDisplay().getSystemColor(SWT.INHERIT_DEFAULT));
		}
	}
}
